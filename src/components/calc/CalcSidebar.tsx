...
export function CalcSidebar({ activeId, onSelect }: CalcSidebarProps) {
  return (
    <>
      {/* Desktop sidebar */}
      <aside className="hidden md:flex flex-col w-[240px] lg:w-[240px] md:w-[200px] flex-shrink-0 bg-sb-0 border-r border-sb-3 h-full overflow-y-auto">
        <div className="px-4 pt-5 pb-3">
          <span className="font-syncopate font-bold text-[11px] text-white tracking-[0.08em]">
            CALCULATORS
          </span>
        </div>
        <div className="w-full h-px bg-sb-3" />
                ...
                return (
                  <button
                    ...
                    className={`w-full text-left px-4 py-2 flex items-center gap-2 transition-colors duration-150 ${
                      isActive
                        ? "bg-sb-1 text-y border-l-2 border-y"
                        : "text-[rgba(255,255,255,0.40)] hover:text-[rgba(255,255,255,0.65)] border-l-2 border-transparent"
                    }`}
                  >
                    <span className="font-mono text-[10px] tracking-[0.04em] leading-tight truncate">
                      {tab.label}
                    </span>
                    <span
                      className={`font-mono text-[7px] tracking-[0.16em] px-1.5 py-0.5 rounded-sm flex-shrink-0 ${
                        isActive
                          ? "bg-[rgba(242,183,5,0.12)] text-y"
                          : "bg-[rgba(255,255,255,0.04)] text-[rgba(255,255,255,0.22)]"
                      }`}
                    >
                      {tab.badge}
                    </span>
        ...
        <div className="px-4 py-4 border-t border-sb-3">
      ...
      <div className="md:hidden w-full bg-sb-0 border-b border-sb-3 overflow-x-auto">
            ...
            g.tabs.map((tab) => {
              ...
              return (
                <button
                  key={tab.id}
                  onClick={() => onSelect(tab.id)}
                  className={`px-3 py-3 font-mono text-[9px] tracking-[0.08em] whitespace-nowrap transition-colors border-b-2 ${
                    isActive
                      ? "text-y border-y bg-sb-1"
                      : "text-[rgba(255,255,255,0.40)] border-transparent hover:text-[rgba(255,255,255,0.65)]"
                  }`}
                >
                  {tab.label}
                </button>
