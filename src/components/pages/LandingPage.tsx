...
const PRODUCTS_TEASER = [
  { model: "WR-A120", domain: "Air", power: "1.2 kW", weight: "0.42 kg", efficiency: "96.2%" },
  { model: "WR-W480", domain: "Water", power: "4.8 kW", weight: "3.8 kg", efficiency: "94.8%" },
  { model: "WR-L150", domain: "Land", power: "15 kW", weight: "8.2 kg", efficiency: "95.4%" },
  { model: "WR-R024", domain: "Robotics", power: "240 W", weight: "0.68 kg", efficiency: "93.1%" },
];
  ...
  return (
    <>
      ...
      <section className="relative h-screen bg-sb-0 flex items-center overflow-hidden">
        ...
        <div className="page-gutter relative z-10 grid grid-cols-1 lg:grid-cols-5 gap-8 items-center">
          {/* Left 60% — Text */}
        ...
        <div className="page-gutter">
          ...
          <RevealWrapper delay={0.22}>
              ...
              <table className="w-full bg-white border border-dw-3 rounded-[4px]">
                <thead>
                  <tr className="border-b border-dw-3">
                    ...
                    <th className="font-mono text-[9px] tracking-[0.22em] uppercase text-[#8A8A96] text-left px-4 py-3">Domain</th>
                    <th className="font-mono text-[9px] tracking-[0.22em] uppercase text-[#8A8A96] text-left px-4 py-3">Power</th>
                ...
                <tbody>
                  {PRODUCTS_TEASER.map((p) => (
                    <tr key={p.model} className="border-b border-dw-3 last:border-0 hover:bg-[rgba(242,183,5,0.03)] transition-colors">
                      <td className="font-mono text-[13px] text-sb-0 px-4 py-3.5">{p.model}</td>
                      <td className="px-4 py-3.5">
                        <span className={`font-mono text-[8px] tracking-[0.18em] uppercase border px-[7px] py-[3px] ${
                          p.domain === "Air" ? "text-domain-air border-domain-air" :
                          p.domain === "Water" ? "text-domain-water border-domain-water" :
                          p.domain === "Land" ? "text-y border-y" :
                          "text-domain-robotics border-domain-robotics"
                        }`}>
                          {p.domain}
                        </span>
                      </td>
                      <td className="font-mono text-[13px] text-sb-0 px-4 py-3.5">{p.power}</td>
            ...
            <div className="grid grid-cols-1 gap-4 md:hidden">
              {PRODUCTS_TEASER.map((p) => (
                <div key={p.model} className="bg-white border border-dw-3 rounded-[4px] p-4">
                  <div className="flex justify-between items-center mb-4">
                    <span className="font-mono text-[13px] font-bold text-sb-0">{p.model}</span>
                    <span className={`font-mono text-[8px] tracking-[0.18em] uppercase border px-[7px] py-[3px] ${
                      p.domain === "Air" ? "text-domain-air border-domain-air" :
                      p.domain === "Water" ? "text-domain-water border-domain-water" :
                      p.domain === "Land" ? "text-y border-y" :
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      ...
                      <span className="font-mono text-[13px] text-sb-0">{p.power}</span>
                    </div>
                    <div>
                    ...
                    <div>
                      ...
                      <span className="font-mono text-[13px] text-sb-0">{p.efficiency}</span>
                    </div>
                  </div>
                  <Link
                    href="/products"
