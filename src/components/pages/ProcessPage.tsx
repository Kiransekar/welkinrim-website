...
export function ProcessPage() {
  return (
    <>
      {/* Engineering Process — Dark */}
      <section className="bg-sb-0 pt-sp8 pb-sp7">
        <div className="page-gutter">
          <RevealWrapper>
            <SectionOverline text="HOW WE ENGINEER" variant="dark" />
          </RevealWrapper>
          <RevealWrapper delay={0.1}>
            <h1 className="font-syncopate font-bold text-[clamp(48px,7.5vw,104px)] leading-[0.86] tracking-[-0.01em] text-white mb-4">
              PROCESS
            </h1>
          </RevealWrapper>
          <RevealWrapper delay={0.22}>
            <p className="font-work text-[clamp(15px,1.4vw,18px)] leading-[1.78] text-[rgba(255,255,255,0.62)] max-w-[560px] mb-16">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-5 gap-6 lg:gap-4">
            {ENGINEERING_STEPS.map((step, i) => (
              <RevealWrapper key={step.number} delay={i * 0.08}>
                <div className="bg-sb-1 border border-sb-3 rounded-[4px] p-5 h-full flex flex-col">
                  <div className="flex items-center gap-3 mb-4">
                    <span className="font-mono text-[28px] text-y font-bold leading-none">
                      {step.number}
                    </span>
                    {i < ENGINEERING_STEPS.length - 1 && (
                      <div className="hidden lg:block flex-1 h-[1px] bg-sb-3" />
                    )}
                  </div>
                  <h3 className="font-syncopate font-bold text-[20px] text-white mb-3">
                    {step.title}
                  </h3>
                  <p className="font-work text-[13px] leading-[1.72] text-[rgba(255,255,255,0.62)] mb-4 flex-1">
                    {step.description}
                  </p>
                  <ul className="flex flex-col gap-1.5">
                    {step.details.map((detail) => (
                      <li
                        key={detail}
                        className="font-mono text-[9px] tracking-[0.12em] text-[rgba(255,255,255,0.35)] flex items-center gap-2"
                      >
      ...
      <section data-theme="light" className="bg-dw-0 py-sp8">
        <div className="page-gutter">
          ...
          <RevealWrapper delay={0.22}>
            <p className="font-work text-[clamp(13px,1.1vw,15px)] leading-[1.72] text-[#44444C] max-w-[560px] mb-12">
                ...
                <div className="flex gap-6 lg:gap-12 py-8 border-b border-dw-3 last:border-0">
                  ...
                  <div className="flex-1">
                    <div className="flex items-baseline justify-between gap-4 mb-2">
                      <h3 className="font-syncopate font-bold text-[clamp(16px,1.8vw,24px)] text-sb-0">
                        {step.title}
                      </h3>
                    ...
                    <p className="font-work text-[clamp(13px,1.1vw,15px)] leading-[1.72] text-[#44444C] max-w-[600px]">
                      {step.description}
                    </p>
                  </div>
                </div>
          ...
          <RevealWrapper delay={0.4}>
            <div className="mt-12 flex justify-center">
              <Link
                href="/contact"
                className="inline-flex items-center gap-2 font-mono text-[10px] tracking-[0.24em] uppercase border border-sb-0 text-sb-0 bg-transparent px-[32px] py-[14px] hover:bg-sb-0 hover:text-white transition-all duration-200 ease-precise"
              >
